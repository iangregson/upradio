"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var ChannelInfo_component_html_1 = __importDefault(require("./ChannelInfo.component.html"));
var __1 = require("..");
var SWH_IMG = '';
var Identicon = require('identicon.js');
var ChannelInfoMode;
(function (ChannelInfoMode) {
    ChannelInfoMode["READ"] = "READ";
    ChannelInfoMode["READ_WRITE"] = "READ_WRITE";
})(ChannelInfoMode = exports.ChannelInfoMode || (exports.ChannelInfoMode = {}));
var ChannelInfo = /** @class */ (function (_super) {
    __extends(ChannelInfo, _super);
    function ChannelInfo(parent) {
        var _this = _super.call(this, parent, 'UpRadioChannelInfo', ChannelInfo_component_html_1.default) || this;
        _this._mode = ChannelInfoMode.READ;
        _this.parent.classList.add('flex');
        _this.parent.classList.add('flex-col');
        _this.parent.classList.add('flex-grow');
        _this.container.classList.add('flex');
        _this.container.classList.add('flex-col');
        _this.container.classList.add('flex-grow');
        _this.channelInfo = {};
        _this.placeholderBox = _this.container.querySelector('div#channelImagePlaceholder');
        _this.imgBox = _this.container.querySelector('div#channelImage');
        _this.nameBox = _this.container.querySelector('div#channelName');
        _this.nameH1 = _this.nameBox.querySelector('h1');
        _this.descriptionBox = _this.container.querySelector('div#channelDescription');
        _this.descriptionP = _this.descriptionBox.querySelector('p');
        _this.infoBtn = _this.nameBox.querySelector('button#expandInfoBtn');
        _this.infoBtn.onclick = function () { return _this.descriptionBox.classList.toggle('hidden'); };
        _this.editBtn = _this.nameBox.querySelector('button#editInfoBtn');
        return _this;
        // setTimeout(() => {
        //   this.init({
        //     id: 'scotswhayhae',
        //     peerId: uuid(),
        //     name: 'Scots Whay Hae!',
        //     description: `Talking about Scottish culture so you don't have to`
        //   });
        // }, 1500)
        // setTimeout(() => {
        //   this.init({
        //     id: 'scotswhayhae',
        //     peerId: uuid(),
        //     name: 'Scots Whay Hae!',
        //     image: SWH_IMG,
        //     description: `Talking about Scottish culture so you don't have to`
        //   });
        // }, 3000)
    }
    ChannelInfo.prototype.init = function (channelInfo) {
        channelInfo.name && (this.name = channelInfo.name);
        channelInfo.id && (this.id = channelInfo.id);
        channelInfo.peerId && (this.peerId = channelInfo.peerId);
        channelInfo.peerId && this.makeAvatar();
        channelInfo.image && (this.image = channelInfo.image);
        channelInfo.description && (this.description = channelInfo.description);
    };
    ChannelInfo.prototype.makeAvatar = function () {
        if (!this.peerId)
            return;
        var avatarOptions = {
            background: [255, 255, 255, 180],
            margin: 0.2,
            size: 420
        };
        this.avatar = new Identicon(this.peerId, avatarOptions).toString();
    };
    Object.defineProperty(ChannelInfo.prototype, "avatar", {
        get: function () {
            return this.channelInfo.avatar;
        },
        set: function (base64Img) {
            this.channelInfo.avatar = base64Img;
            // this.placeholderBox.innerHTML = `<img class="rounded main-mode-icon" src="data:image/svg+xml;base64,${this.avatar}">`;
            this.placeholderBox.innerHTML = "<img class=\"rounded main-mode-icon\" src=\"data:image/png;base64," + this.avatar + "\">";
            var img = "url('data:image/png;base64, " + this.avatar + "')";
            this.imgBox.style.backgroundImage = img;
            this.imgBox.classList.remove('hidden');
            this.placeholderBox.classList.add('hidden');
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "image", {
        get: function () {
            return this.channelInfo.image;
        },
        set: function (base64Img) {
            this.channelInfo.image = base64Img;
            var img = "url('data:image/png;base64, " + base64Img + "')";
            this.imgBox.style.backgroundImage = img;
            this.imgBox.classList.remove('hidden');
            this.placeholderBox.classList.add('hidden');
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "name", {
        get: function () {
            return this.channelInfo.name;
        },
        set: function (channelName) {
            this.channelInfo.name = channelName;
            this.nameH1.innerText = channelName;
            this.nameBox.classList.remove('hidden');
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "channelId", {
        get: function () {
            return this.channelInfo.id;
        },
        set: function (id) {
            this.channelInfo.id = id;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "peerId", {
        get: function () {
            return this.channelInfo.peerId;
        },
        set: function (peerId) {
            this.channelInfo.peerId = peerId;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "description", {
        get: function () {
            return this.channelInfo.description;
        },
        set: function (description) {
            this.channelInfo.description = description;
            this.descriptionP.innerText = description;
            this.isReadOnly && this.infoBtn.classList.remove('hidden');
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "mode", {
        get: function () {
            return this._mode;
        },
        set: function (channelInfoMode) {
            this._mode = channelInfoMode;
            if (this._mode === ChannelInfoMode.READ_WRITE) {
                this.infoBtn.classList.add('hidden');
                this.editBtn.classList.remove('hidden');
                this.nameBox.classList.remove('hidden');
            }
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ChannelInfo.prototype, "isReadOnly", {
        get: function () {
            return this.mode === ChannelInfoMode.READ;
        },
        enumerable: true,
        configurable: true
    });
    return ChannelInfo;
}(__1.Component));
exports.ChannelInfo = ChannelInfo;
